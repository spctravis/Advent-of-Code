        ...
        if($point -ne $null){
            ...
            if($point.line -ne 0){
                ...
                if($aboveline[($point-1)..($point+1)] -match '\d'){
                    $aboveline[($point-3)..($point+3)] | foreach { if($_ -match '\d'){$num+=$_}}
                    $total += $num
                    $num = 0
                }
            }#end if
            ...
            if($sameline[($point-1)] -match '\d'){
                $sameline[($point-3)..($point-1)] | foreach { if($_ -match '\d'){$num+=$_}}
                $total += $num
                $num = 0
            }
            #same line
            #right
            if($sameline[($point+1)] -match '\d'){
                $sameline[($point+1)..($point+3)] | foreach { if($_ -match '\d'){$num+=$_}}
                $total += $num
                $num = 0
            }
            #below line
            if($point.line -ne $countoflines){
                ...
                if($belowline[($point-1)..($point+1)] -match '\d'){
                    $belowline[($point-3)..($point+3)] | foreach { if($_ -match '\d'){$num+=$_}}
                    $total += $num
                    $num = 0
                }#end if
            } #end if
